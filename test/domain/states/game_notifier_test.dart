...
void main() {
  ...
  group('playAgain', () {
    test(
      'should reset the game state. Previous loser should start the game.',
      () {
        final container = ProviderContainer.test();

        expect(container.read(gameProvider).currentPlayer, Player.X);
        container.read(gameProvider.notifier).makeMove(0);
        expect(container.read(gameProvider).currentPlayer, Player.O);
        container.read(gameProvider.notifier).makeMove(1);
        container.read(gameProvider.notifier).makeMove(3);
        container.read(gameProvider.notifier).makeMove(4);
        container.read(gameProvider.notifier).makeMove(6);

        expect(container.read(gameProvider).winner, Player.X);

        container.read(gameProvider.notifier).playAgain();
        final stateAfterReset = container.read(gameProvider);

        expect(stateAfterReset.board, equals(List.filled(9, Player.none)));
        expect(stateAfterReset.isGameOver, isFalse);
        expect(stateAfterReset.winner, isNull);
        expect(stateAfterReset.currentPlayer, equals(Player.O));
        ...
        expect(stateAfterSecondReset.isGameOver, isFalse);
        expect(stateAfterSecondReset.winner, isNull);
        expect(stateAfterSecondReset.currentPlayer, equals(Player.X));
      },
    );
  });
